...
func List(c echo.Context) error {
	...
	if int64(nextPage*pageSize) <= total {
		...
		nextValues.Set("page_size", strconv.Itoa(pageSize))
		if sort != "" {
	...
	if prevPage > 0 {
		...
		prevValues.Set("page_size", strconv.Itoa(pageSize))
		if sort != "" {
...
//	@Param			max_dispatch_pre_hour	formData	int		true	"Max dispatch per hour"
...
func Store(c echo.Context) error {
	...
	task.ContextType = c.FormValue("context_type")
	task.State = c.FormValue("state")
	maxDispatchPreHour, err := strconv.Atoi(c.FormValue("max_dispatch_pre_hour"))
	if err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Invalid max_dispatch_pre_hour"})
	}
	task.MaxDispatchPreHour = maxDispatchPreHour
...
//	@Param			max_dispatch_pre_hour	formData	int		false	"Max dispatch per hour"
//	@Success		200						{object}	models.Task
//	@Failure		400						{object}	map[string]string
//	@Failure		404						{object}	map[string]string
//	@Failure		500						{object}	map[string]string
...
func Update(c echo.Context) error {
	...
	if maxDispatchPreHourStr := c.FormValue("max_dispatch_pre_hour"); maxDispatchPreHourStr != "" {
		maxDispatchPreHour, err := strconv.Atoi(maxDispatchPreHourStr)
		if err != nil {
			return c.JSON(http.StatusBadRequest, map[string]string{"error": "Invalid max_dispatch_pre_hour"})
		}
		existingTask.MaxDispatchPreHour = maxDispatchPreHour
	}
...
//	@Failure		500	{object}	map[string]string
//	@Security		JWT
//	@Router			/tasks/{id}/start-immediately [post]
func StartImmediately(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, map[string]string{
		...
		"task_id":     strconv.FormatUint(uint64(task.ID), 10),
		"schedule_at": now.Format(time.RFC3339),
	})
...
//	@Failure		400		{object}	map[string]string
//	@Security		JWT
//	@Router			/tasks/content-template [get]
...
//	@Security		JWT
//	@Router			/tasks/receivers-template [get]
