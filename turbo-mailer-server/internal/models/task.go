...
type Task struct {
	...
	Pools              []TaskPool `gorm:"many2many:task_pools;" json:"pools"`
	MaxDispatchPreHour int        `gorm:"column:max_dispatch_pre_hour;type:int;not null" json:"max_dispatch_pre_hour"`
	LastDispatchAt     *time.Time `gorm:"column:last_dispatch_at;type:datetime;not null" json:"last_dispatch_at"`
}
...
type TaskLogArchive struct {
	TaskLog
	ArchivedAt time.Time `gorm:"column:archived_at;type:datetime;not null" json:"archived_at"`
}
